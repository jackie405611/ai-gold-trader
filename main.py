...
def _process_symbol(symbol, cfg, prev_counts):
    ...
    # Trailing stop ทำทุก loop เสมอ (แม้ตลาดจะ "ปิด" สำหรับ symbol อื่น)
    ...
    # ── Session filter ต่อ symbol (แต่ละตัวมี sessions ต่างกัน) ──
    if not session_ok(cfg):
        print(f"[{symbol}] ⏸  Outside session — skip")
        return
    ...
    # ── ถ้ามี position อยู่แล้ว ไม่เปิดเพิ่ม (แต่ trailing ยังทำอยู่ด้านบน) ──
...
def main():
    ...
    while ctrl.is_bot_running():
        try:
            ...
            # ── Weekend gate สำหรับ Forex/Gold เท่านั้น ──
            # BTC และ crypto เปิด 24/7 จึงไม่ใช้ global market_open()
            # แต่ละ symbol ดูแล session ตัวเองใน _process_symbol → session_ok(cfg)
