# ============================================================
#  lib/chart_generator.py  —  Chart with RSI + EMA (Vercel)
#  Adapted: accepts pre-fetched DataFrame, returns bytes
#  (no MT5, no disk I/O)
# ============================================================
import io
import pandas as pd
...
def generate_chart(df: pd.DataFrame, symbol: str = "XAUUSDm",
    ...
    """
    Build a 2-panel chart (Price + EMA9/21/50, RSI) from a DataFrame.

    df          : OHLCV DataFrame with DatetimeIndex (from data_fetcher)
    symbol      : display name
    signal_info : optional dict from ai_strategy (adds title annotation)
    Returns     : PNG image as bytes (send directly to Telegram)
    """
    ...
    ax1.plot(df.index, df["ema9"],  color="#f0883e", linewidth=1.0, label="EMA9")
    ax1.plot(df.index, df["ema21"], color="#3fb950", linewidth=1.0, label="EMA21")
    ax1.plot(df.index, df["ema50"], color="#bc8cff", linewidth=1.0, linestyle="--", label="EMA50")

    title = f"{symbol} M5"
    if signal_info:
        title += (f"  |  Signal: {signal_info.get('m5_signal', '')}"
                  f"  |  Regime: {signal_info.get('regime', '')}"
